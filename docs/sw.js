...
const CACHE_VERSION = 'bundesliga-v2';
const DATA_CACHE    = 'bundesliga-data-v2';
const FLAG_CACHE    = 'bundesliga-club-images-v1';
...
self.addEventListener('fetch', event => {
  ...
  // Only handle GET requests for our own origin + known club-image hosts.
  ...
  const isImageCDN = imageHosts.has(url.hostname);
  if (url.origin !== self.location.origin && !isImageCDN) return;

  if (DATA_FILES.some(f => url.pathname.endsWith(f))) {
    // Competition JSON: network-first → fresh data online, cached offline
    event.respondWith(networkFirst(request, DATA_CACHE));
    return;
  }

  if (isImageCDN) {
    // Club crests: stale-while-revalidate — serve cached, refresh in background
...
async function networkFirst(request, cacheName) {
  ...
  try {
    ...
    return withDataSource(res, 'network');
  } catch {
    const cached = await caches.match(cacheKey);
    return cached ? withDataSource(cached, 'cache') : new Response(JSON.stringify({ error: 'offline' }), {
      status: 503,
...
function withDataSource(response, source) {
  const headers = new Headers(response.headers);
  headers.set('X-Bundesliga-Data', source);
